...
from pgmpy.estimators import K2Score, HillClimbSearch, MaximumLikelihoodEstimator, BDeuScore, AICScore
...
util.print_BN(dag, vis_ls=["circo"])

scoring_method = K2Score(data=samples)  # BDeuScore | AICScore
estimator = HillClimbSearch(data=samples)

dag: pgmpy.base.DAG = estimator.estimate(
    scoring_method=scoring_method, max_indegree=4, epsilon=5000,
)
